...
- **Load card** (bordered, radius 20, two equal cells split by a hairline):
  - Left: **"WEEKLY LOAD"** eyebrow → big tabular volume ("28,280 lb", 25px/800,
    `"lb"` 13px/600 secondary) → sub-label ("12 sets last week").
  - Right: **"LIFETIME LOAD"** → volume ("364,525 lb") → ("4,156 total sets").
  - No progress ring, no percent, no delta (those were an earlier iteration).
...
- **Straight sets / Rounds segmented control** (inset pill, radius ~12, gold
  active segment with ink text): switches the session between plain straight-set
  logging and **rounds/circuit** mode (see behavior E below).
...
## E. Rounds / circuit mode (Active Workout)
The active-workout screen has a **"Straight sets" / "Rounds"** segmented control.
- **Straight sets** (default): sets are logged per exercise as usual.
- **Rounds**: the session groups exercises into circuit **rounds** — `session`
  carries `roundsMode:true` and `currentRound` (starts at 1); a **"Next round"**
  action increments it (toast "Round N"), and logged sets are grouped/expandable
  per round (`expandedRounds`). Individual exercises can be flagged `circuit`.
- The default mode comes from the profile preference **`roundsDefault`**
  (`'straight'` | `'rounds'`); templates flagged `circuit` start in rounds mode.
- Exercise cards can be **collapsed** (`collapsedEx` map) via the chevron in the
  card header.
Exact state shape and handlers (`setRoundsMode`, `nextRound`, `toggleRound`,
`toggleCircuit`, `toggleCollapse`) are in `index.dc.html`.
...
- `screenshots/` — current captures: 01 reminder · 02 home · 03 history ·
  04 records · 05 profile · 06 active-workout.
